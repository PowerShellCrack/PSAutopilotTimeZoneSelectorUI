...
UserDriven | Boolean | _True_ | Deploy to user when set to true. if _true_ sets HKCU key, if _false_, set HKLM key. | Set to _$True_ if the deployment is for autopilot. Non _Users context_ deployment will need permission to write to HKLM registry hive
RunOnce | Boolean |  _True_ | Specifies this script will only launch one time. If its user-driven it will launch once per user on one device. If its device driven, it will only launch once for a user on the device.| RunOnce is determined by monitoring a custom registry entry. This will not work if NoControl is set to _$True_
